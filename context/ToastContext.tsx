...
export function ToastProvider({ children }: { children: ReactNode }) {
  ...
  const getToastStyles = (type: Toast['type']) => {
    const baseStyles = "flex items-center p-4 mb-3 text-sm rounded-lg shadow-lg transform transition-all duration-300 ease-in-out"
  ...
  const getIcon = (type: Toast['type']) => {
    switch (type) {
      case 'success':
        return (
          <svg className="flex-shrink-0 w-4 h-4 mr-3" fill="currentColor" viewBox="0 0 20 20">
        ...
        {toasts.map((toast) => (
          ...
          >
            ...
            <button
              ...
              className="ml-3 text-current hover:text-gray-600 focus:outline-none"
            >
